			...
			if (activity != null) {
				switch (msg.what) {
					...
					case cBluetooth.BL_SOCKET_FAILED:
						...
						activity.finish();
						break;
					case cBluetooth.RECIEVE_MESSAGE:								// if message is recieved (åñëè ñîîáùåíèå ïîëó÷åíî)
						byte[] readBuf = (byte[]) msg.obj;
						String strIncom = new String(readBuf, 0, msg.arg1);
						sb.append(strIncom);										// append string (ïðèñîåäèíÿåì ñòðîêó)

						int FDataLineIndex = sb.indexOf("FData:");					// string with Flash-data (ñòðîêà ñ Flash äàííûìè (÷òåíèå))
						int FWOKLineIndex = sb.indexOf("FWOK");						// string with the message of the successful record in Flash (ñòðîêà ñ ñîîáùåíèåì îá óñïåøíîé çàïèñè â Flash)
